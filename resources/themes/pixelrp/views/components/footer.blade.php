...
<footer class="mt-auto w-full bg-(--color-ink-soft) border-t-4 border-(--color-coin)">
    <div class="mx-auto flex max-w-7xl items-center justify-between px-8 py-3.5 text-[11px] font-bold tracking-[1.5px] uppercase">
        ...
        <div class="flex items-center gap-2"
            ...
            <span class="pt-server-dot" :class="{ 'pt-server-dot--updating': updating }"></span>
            <span class="text-white"
                  x-text="updating ? {{ \Illuminate\Support\Js::from(__('All servers updating')) }} : {{ \Illuminate\Support\Js::from(__('All servers online')) }}">{{ $isUpdating ? __('All servers updating') : __('All servers online') }}</span>
        </div>
    </div>
</footer>
