@props(['eyebrow' => 'Pixel Tower Digital', 'heading', 'sub' => null])

<section class="flex w-full max-w-md flex-col gap-6 text-white">
    <div class="flex flex-col gap-3">
        <span class="pt-eyebrow">{{ $eyebrow }}</span>
        <h1 class="text-[40px] font-black leading-[1.05] tracking-tight">{{ $heading }}</h1>
        @if ($sub)
            <p class="max-w-md text-[14px] font-medium leading-relaxed text-(--color-hero-sub)">{{ $sub }}</p>
        @endif
    </div>
    ...
    <ul class="flex flex-col gap-3">
        @foreach (['Manage your character and inventory', 'Track gang reputation and territory', 'Grind shifts at your favorite local business', 'Tend crops and read the town ledger'] as $item)
            <li class="flex items-center gap-3 text-[14px] font-semibold text-white">
